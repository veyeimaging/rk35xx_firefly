...
static struct v4l2_ctrl_config mvcam_v4l2_ctrls[] = {
    //standard v4l2_ctrls
    {
		...
		.type = V4L2_CTRL_TYPE_MENU ,//should be menu		
        .def = 0,
		.min = 0,
...
VEYE_TRACE
   // mutex_init(&mvcam->mutex);
    ...
    for (i = 0; i < ARRAY_SIZE(mvcam_v4l2_ctrls); ++i) {
        ...
        dev_info(&client->dev, "init control %s success\n",mvcam_v4l2_ctrls[i].name);
	}
			...
			const struct i2c_device_id *id)
{
    ...
    mvcam->reset_gpio = devm_gpiod_get(dev, "reset", GPIOD_OUT_LOW);
